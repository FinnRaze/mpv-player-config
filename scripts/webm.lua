...
do
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      ass:new_event()
      self:setup_text(ass)
      ass:append(tostring(bold('裁剪:')) .. "\\N")
      ass:append(tostring(bold('1:')) .. " 改变点A (" .. tostring(self.pointA.x) .. ", " .. tostring(self.pointA.y) .. ")\\N")
      ass:append(tostring(bold('2:')) .. " 改变点B (" .. tostring(self.pointB.x) .. ", " .. tostring(self.pointB.y) .. ")\\N")
      ass:append(tostring(bold('r:')) .. " 重设为全屏\\N")
      ass:append(tostring(bold('ESC:')) .. " 取消裁剪\\N")
      local width, height = math.abs(self.pointA.x - self.pointB.x), math.abs(self.pointA.y - self.pointB.y)
      ass:append(tostring(bold('ENTER:')) .. " 确认裁剪 (" .. tostring(width) .. "x" .. tostring(height) .. ")\\N")
...
do
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      ass:new_event()
      self:setup_text(ass)
      ass:append(tostring(bold('选项:')) .. "\\N\\N")
      ...
      ass:append("\\N▲ / ▼: 导航\\N")
      ass:append(tostring(bold('ENTER:')) .. " 确认选项\\N")
      ass:append(tostring(bold('ESC:')) .. " 取消\\N")
      return mp.set_osd_ass(window_w, window_h, ass.text)
    end
  }
  _base_0.__index = _base_0
  ...
  _class_0 = setmetatable({
    __init = function(self, callback)
      ...
      self.options = {
        {
          "output_format",
          Option("list", "输出格式", options.output_format, formatOpts)
        },
        {
          "twopass",
          Option("bool", "二次编码", options.twopass)
        },
        {
          "apply_current_filters",
          Option("bool", "应用目前视频滤镜", options.apply_current_filters)
        },
        {
          "scale_height",
          Option("list", "缩放高度", options.scale_height, scaleHeightOpts)
        },
        {
          "strict_filesize_constraint",
          Option("bool", "严格文件大小控制", options.strict_filesize_constraint)
        },
        {
          "write_filename_on_metadata",
          Option("bool", "带元数据文件名", options.write_filename_on_metadata)
        },
        {
          "target_filesize",
          Option("int", "目标文件大小", options.target_filesize, filesizeOpts)
        },
        {
          "crf",
        ...
        {
          "gif_dither",
          Option("list", "GIF 抖动算法类型", options.gif_dither, gifDitherOpts, function()
            return self.options[1][2]:getValue() == "gif"
          end)
        },
        {
          "force_square_pixels",
          Option("bool", "强制方形像素", options.force_square_pixels)
        }
      }
      self.keybinds = {
        ["LEFT"] = (function()
...
do
  ...
  local _base_0 = {
    ...
    draw = function(self)
      ...
      ass:append(tostring(bold('c:')) .. " 裁剪\\N")
      ass:append(tostring(bold('1:')) .. " 设置起始时间 (目前是 " .. tostring(seconds_to_time_string(self.startTime)) .. ")\\N")
      ass:append(tostring(bold('2:')) .. " 设置结束时间 (目前是 " .. tostring(seconds_to_time_string(self.endTime)) .. ")\\N")
      ass:append(tostring(bold('o:')) .. " 改变编码选项\\N")
      ass:append(tostring(bold('p:')) .. " 预览\\N")
      ass:append(tostring(bold('e:')) .. " 编码\\N\\N")
      ass:append(tostring(bold('ESC:')) .. " 关闭\\N")
      return mp.set_osd_ass(window_w, window_h, ass.text)
    end,
    show = function(self)
